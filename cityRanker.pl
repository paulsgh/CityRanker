...
%% 
% categories constants on which user can query
%% 
category("housing", 0).
category("cost", 1).
category("startups", 2).
...
% gives user an introduction to the program with instructions on how to enter input
main_input("intro") :-
    nl,
...
% lists the cities when prompted in the main menu
main_input("list") :-
    list_cities(),
    nl,
    main_options().

% directs the user to the main querying options
main_input("skip") :-
    main_options().

% erroneous input, directs the user back to the main menu
main_input(_) :-
    nl,
...
% when user wants to know info from one city only.
...
% when user wants to compare two cities' info.
...
% errnoeous input, redirects user to main_options
...
% prints the scores of two cities in one category and declares them to be the same
compare_two_cities(C1, S1, C2, S2):-
    S1=S2,
...
% if erroneous input, redirects to continuation options once again.
continue_input("Yes") :-
    nl,
    nl,
    main_options().
...
% true if Score is the rating for City based on Feature, and V is thevisualization of Score
...
% calls helpers to print list of all valid cities.
list_cities() :-
...
% organizes the cities to be output in three columns when printed
...
%% check that input category is a valid category (when querying 1 city).
%% if so, continue program. otherwise, return to 1 city query options.
validate_category(Category, Location) :-
...
%% check that input category is a valid category (when comparing 2 cities).
%% if so, continue program. otherwise, return to 2 city query options.
validate_category_two(Category, City1, City2) :-
    Categories = ["housing", "cost", "startups", "travel", "commute", "business", "safety", "healthcare", "education",
                  "environment", "economy", "tolerance", "taxation", "internet", "culture", "outdoors"],
